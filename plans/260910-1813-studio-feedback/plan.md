...
Status: implementation and local verification complete; CI and release pending
...
## Work
- [x] Shared public navigation, home decoration, project thumbnails, route/history state.
- [x] Layers DnD, component icons/properties, collapsible editor panes, preview toggle.
- [x] Material/texture tools, camera/light layout, 3D/2D composition order.
- [x] Motion workspace and Space playback; presentation controls and contrast.
- [x] Additional use-case templates and well-known system-inspired presets.
- [x] Owning docs and generated discovery synchronization.
...
Main gained provider integrations (PR #13, `ee7ab1b`) during finalization. Merged those changes preserving provider settings and both documentation updates. This release advances to 0.3.2; combined verification passed unit/type/build and focused regression/browser checks; clean full-suite CI remains required before merge. AgentKit CLI is unavailable, so no plan registry transition or CLI-persisted journal is claimed.
