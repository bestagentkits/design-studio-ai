...
Status: in progress. Priority: P1. Provisional effort: 6–10 engineering days.
...
Current delivery update: Static saved posters, live GIF board order/clipping and exact-time raster sampling are implemented; see [Elements runtime evidence](reports/elements-runtime-delivery.md). The full format/publication matrix and release verification are not yet closed.
...
- [x] Build a render projection from selected pages/boards/artwork and exact contributing generations. Use derived composites for static paint instead of embedding every editable/historical tile; regenerate or fail mismatched composites.
- [x] Render all Board paths/text/connectors/labels/roughness/seed, nested transforms/crops, paint blends/masks and selected GIF samples consistently. Preserve finite export bounds while camera navigation stays unbounded-feeling.
...
- [x] Extend trusted HTML/published viewer/React prototype runtime with owned or packaged assets and controlled GIF playback/reduced motion; no external app iframe, SDK activation or private project fetches.
- [x] Sample GIF time deterministically in WebM/supported MP4 using existing duration/pixel limits and encoder errors. Keep browser fallback audio limitations explicit.
- [x] Retain Google Slides native/HTTPS-image policy: unsupported Board/Paint fails preflight; never publish private images automatically. GLB/glTF retains actual 3D behavior and explicitly identifies unsupported 2D content.
