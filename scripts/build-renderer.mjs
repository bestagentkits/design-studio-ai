...
const sourceFiles = ['src/app/document-view.tsx', 'src/app/design-component.tsx', 'src/shared/schema.ts', 'src/shared/design-capabilities.ts', 'src/shared/layout.ts', 'src/shared/render.ts', 'src/shared/easing.ts', 'src/shared/font-loading.ts'];
...
const dependencies = Object.fromEntries(Object.entries(pkg.dependencies).filter(([name]) => ['react', 'react-dom', 'antd', 'zod'].includes(name) || name.startsWith('@radix-ui/')));
