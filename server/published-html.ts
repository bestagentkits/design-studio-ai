...
export async function interactiveHtml(c: Context<Env>, doc: DesignDocument, nonce?: string) {
  if (doc.kind !== 'slides' && !doc.timeline && !doc.pages.some(page => page.layout || page.nodes.some(node => node.layout || node.component || node.type === 'model3d' && node.visible !== false))) return renderHtml(doc);
