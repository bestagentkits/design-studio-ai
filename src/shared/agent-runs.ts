...
export const agentRunStartSchema = z.strictObject({
  ...
  provider: z.enum(['openai', 'anthropic', 'gemini', 'openrouter']), model: z.string().min(1).max(200).optional(),
...
export const agentRunSchema = z.strictObject({
  ...
  provider: z.enum(['openai', 'anthropic', 'gemini', 'openrouter']), model: z.string().min(1).max(200),
