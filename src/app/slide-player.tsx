...
function Slide({ doc, index, navigate }: { doc: DesignDocument; index: number; navigate: (id: string) => void }) {
...
export function SlidePlayer({ doc, close, notes = false }: { doc: DesignDocument; close?: () => void; notes?: boolean }) {
  ...
  const pageView = (i: number) => <Slide doc={doc} index={i} navigate={id => go(doc.pages.findIndex(p => p.id === id))}/>;
