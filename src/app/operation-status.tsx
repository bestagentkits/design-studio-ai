...
export function OperationStatus({projectId}:{projectId:string}){
 ...
 return <><button className="button small" onClick={()=>setOpen(true)}>Operations</button>{open&&<Modal title="Operations" onClose={()=>setOpen(false)}><div className="modal-body"><label>Operation ID<input value={id} onChange={e=>{setId(e.target.value);setJob(null);}}/></label><button disabled={!id} onClick={check}>Check saved operation</button>{job&&<p>{job.kind}: {job.status} · {job.stage}{job.revision?` · revision ${job.revision}`:''}{job.error?.message}</p>}{job?.resultUrl&&<a href={job.resultUrl} download={`studio-${job.id}-result`}>Download result / save receipt</a>}{error&&<p role="alert">{error}</p>}<p>A disconnected client does not cancel a queued job. Check the receipt before reloading or saving again.</p></div></Modal>}</>;
}
