...
export function CharacterView({character,instance,assets,time,playback=false}:{character:Character;instance:CharacterInstance;assets:{id:string;url:string}[];time:number;playback?:boolean}) {
 ...
 useEffect(()=>{const node=canvas.current;if(!node)return;
   ...
   if(!fallback)try{renderer.current=new CharacterWebGL(node);}catch{setFallback(true);}
 ...
 useEffect(()=>{const node=canvas.current;if(!node)return;try{
   ...
   else if(fallback){const ctx=node.getContext('2d');if(ctx){ctx.clearRect(0,0,node.width,node.height);ctx.save();ctx.scale(node.width/character.width,node.height/character.height);drawCharacter(ctx,character,active,images.current,at);ctx.restore();}}
 }catch(e){setError(e instanceof Error?e.message:'Unable to render character');}},[character,instance,time,loaded,fallback,trigger,interactionTime]);
 ...
 return <div data-character-ready={ready&&!error?'true':'false'} data-character-error={error||undefined} style={{width:'100%',height:'100%'}} onClick={()=>interact('click')} onMouseEnter={()=>interact('hover')}>
  <canvas key={fallback?'canvas':'gpu'} ref={canvas} width={Math.min(2048,character.width)} height={Math.min(2048,character.height)} aria-label={character.name} style={{width:'100%',height:'100%'}}/>
