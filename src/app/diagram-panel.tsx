...
export function DiagramPanel({ doc, boardId, onCommit, selectedIds = [] }: { doc: DesignDocument; boardId: string; onCommit: (base: DesignDocument, next: DesignDocument) => void; selectedIds?: string[] }) {
  ...
  return <section aria-label="Diagram tools" className="panel" onKeyDown={e => {
    ...
    <h3>Diagrams</h3>
    <label>Diagram family<select value={family} onChange={e => { const next = e.target.value as DiagramFamily; setFamily(next); setRole(diagramRoles[next][0]); }}>
    ...
    {selected?.type === 'connector' && <>
      <label>Manual bends (JSON points)<input aria-label="Manual bends" value={bends} onChange={e => setBends(e.target.value)} /></label>
      <button onClick={() => { try { run({ op: 'diagram-edge', boardId, edgeId: selected.id, bends: JSON.parse(bends) }); } catch { setError('Enter a JSON array of x/y points'); } }}>Apply bends</button>
