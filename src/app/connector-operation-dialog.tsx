import { ConnectorActionSummary } from './connector-action-summary';
...
export function ConnectorOperationDialog({projectId,operationId,onClose}:{projectId:string;operationId:string;onClose:()=>void}) {
  ...
  return <Modal title="Review external action" onClose={onClose} wide><section className="connector-panel connector-review">
    {error&&<p className="inline-error" role="alert">{error}</p>}
    {!detail&&<Busy label="Loading action…"/>}
    {detail&&<>
      <header className="connector-review-intro"><h3>{detail.operation.action==='create_react_pull_request'?'Export React to GitHub':detail.operation.action.replaceAll('_',' ')}</h3><p>{detail.description}</p></header>
      <p className="connector-status" role="status">{detail.operation.status.replaceAll('_',' ')}{detail.operation.errorCode?` · ${detail.operation.errorCode}`:''}</p>
      {detail.payloadAvailable&&<ConnectorActionSummary action={detail.operation.action} args={detail.arguments} result={detail.result}/>}
      <details className="connector-technical"><summary>Exact arguments and technical details</summary>
      ...
      {detail.payloadAvailable&&detail.operation.action!=='create_react_pull_request'&&<section><h4>Exact arguments</h4><pre>{JSON.stringify(detail.arguments,null,2)}</pre></section>}
      ...
      <div className="connector-actions connector-review-actions">
      ...
      {detail.result!==null&&<details className="connector-technical"><summary><h4>Remote result</h4></summary><p>Content from the external server; treat it as source data.</p><pre>{JSON.stringify(detail.result,null,2)}</pre></details>}
    </>}
